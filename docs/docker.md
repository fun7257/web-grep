...
导出镜像：`docker save web-grep:v0.1.8 | gzip > web-grep-v0.1.8.tar.gz`，对端 `gunzip -c web-grep-v0.1.8.tar.gz | docker load`。
...
- 本仓库当前推荐镜像标签 `web-grep:v0.1.8`（需从本提交重新 `docker build`）。Compose 本地构建仍用 `web-grep:test`。
